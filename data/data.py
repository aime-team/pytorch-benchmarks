...
class MultiGpuData(object):
    ...
    def load_dataset(self):
        ...
        if self.args.imagenet:
            train_dataset = torchvision.datasets.ImageNet(
                root=self.args.imagenet, transform=MultiGpuData.transforms, split='train'
                                                          )
            val_dataset = torchvision.datasets.ImageNet(
                root=self.args.imagenet, transform=MultiGpuData.transforms, split='val'
                                                        )
            ...
            if self.args.train_folder:
                train_dataset = torchvision.datasets.ImageFolder(
                    root=self.args.train_folder, transform=MultiGpuData.transforms
                                                                 )
            else:
                ...
                train_dataset = RandomDataset(
                    image_size, num_images, num_classes, transform=MultiGpuData.transforms
                                              )
                label_dict = [i for i in range(num_classes)]
            if self.args.val_folder:
                val_dataset = torchvision.datasets.ImageFolder(
                    root=self.args.val_folder, transform=MultiGpuData.transforms
                                                               )            
    ...
    def init_distributed_dataloader(self, dataset, rank):
        ...
        if dataset:
            if self.args.distributed:
                sampler = torch.utils.data.distributed.DistributedSampler(dataset, num_replicas=self.args.num_gpus, rank=rank)
            else:
                sampler = None
            ...
            dataloader = torch.utils.data.DataLoader(
                ...
                shuffle=not self.args.distributed,
    ...
    def load_single_picture(picture_file):
        ...
        with Image.open(picture_file) as img:
            image = MultiGpuData.transforms(img).float()
