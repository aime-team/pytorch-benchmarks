...
class Benchmark(object):
    ...
    def calculate_benchmark(self, rank, epoch, step, total_steps, loss):
        ...
        Returns the new start time for the next iteration.
        """
        prompt_str = ''
    ...
    def finish_epoch(self, rank, epoch, total_steps, loss):
        ...
        if rank == 0:
            ...
            if not self.eval_mode:
                ...
                val_acc = self.calculate_validation_accuracy()
    ...
    def finish_benchmark(self):
        ...
        if self.args.mean_img_per_sec:
            mean_img_per_sec_str = self.make_mean_img_per_sec_string()
        else:
        ...
        final_str = mean_img_per_sec_str + max_temp_str + val_acc_str + f'\nBenchmark end: {end_time}\n'
        if self.args.log_file:
    ...
    def calculate_images_per_sec(self, epoch, step, duration):
        ...
        if self.args.mean_img_per_sec:
            if not self.warm_up_stage:
                if self.eval_mode:
                    self.img_per_sec_dicts[1][(epoch, step)] = img_per_sec
                else:
                    self.img_per_sec_dicts[0][(epoch, step)] = img_per_sec
    ...
    def make_mean_img_per_sec_string(self):
        """Calculates the mean images per second for training and/or evaluation and returns it
        in the shape of a printable string.
        """
        mean_img_per_sec_str = f'\nMean images per second: '
        if len(self.img_per_sec_dicts[0]):
            mean_img_per_sec_train = round(np.array(list(self.img_per_sec_dicts[0].values())).mean())
        else:
            mean_img_per_sec_train = 0
        if len(self.img_per_sec_dicts[1]):
            mean_img_per_sec_eval = round(np.array(list(self.img_per_sec_dicts[1].values())).mean())
        else:
            mean_img_per_sec_eval = 0
        if not self.args.eval_only:
            mean_img_per_sec_str += f'training: {mean_img_per_sec_train}'
            if len(self.img_per_sec_dicts[1]):
                mean_img_per_sec_str += f', '
        if len(self.img_per_sec_dicts[1]):
            mean_img_per_sec_str += f'evaluation: {mean_img_per_sec_eval}'
